import type { Db, PayMethod, Product, PurchaseItem, SaleItem } from './types';
...
type PurchaseSeed = [daysAgo: number, supplierId: number, lines: [number, number, number][]];

const PRODUCTS: ProductSeed[] = [
  ['TS-1001', 'Classic Logo Tee', 1, 1, 6.5, 14.99, 42, 10],
  ['TS-1002', 'Vintage Graphic Tee', 1, 1, 7.25, 16.5, 8, 10],
  ['HD-1101', 'Fleece Pullover Hoodie', 1, 1, 14, 32, 18, 6],
  ['MG-2001', 'Ceramic Shop Mug', 2, 2, 2.8, 8.99, 60, 15],
  ['MG-2002', 'Enamel Camp Mug', 2, 2, 3.6, 11.5, 12, 12],
  ['BT-9001', 'Steel Water Bottle 500ml', 2, 2, 4.8, 13.5, 32, 10],
  ['TT-3001', 'Canvas Tote Bag', 3, 3, 2.2, 7.99, 55, 12],
  ['TT-3002', 'Insulated Lunch Tote', 3, 3, 5.4, 14, 20, 8],
  ['CP-4001', 'Snapback Cap', 4, 3, 3.9, 12, 26, 10],
  ['CP-4002', 'Knit Beanie', 4, 3, 3.1, 9.5, 6, 8],
  ['ST-5001', 'Dot-Grid Notebook A5', 5, 4, 1.9, 5.99, 70, 20],
  ['ST-5002', 'Gel Pen Set (5-pack)', 5, 4, 1.2, 3.99, 90, 25],
  ['KC-6001', 'Acrylic Keychain', 6, 4, 0.8, 3.5, 110, 30],
  ['KC-6002', 'Bottle-Opener Keyring', 6, 4, 1.1, 4.25, 14, 15],
  ['SK-7001', 'Sticker Pack (12 pcs)', 7, 2, 0.95, 3.25, 140, 40],
  ['PT-7002', 'Art Print A4', 7, 2, 1.6, 6, 30, 10],
  ['HL-8001', 'Scented Soy Candle', 8, 4, 3.2, 9.99, 24, 8],
  ['HL-8002', 'Ceramic Coaster Set', 8, 4, 2.7, 8.5, 9, 10],
];

const PURCHASES: PurchaseSeed[] = [
  [42, 1, [[1, 40, 6.5], [3, 15, 14]]],
  [33, 2, [[4, 48, 2.8], [15, 100, 0.95], [16, 25, 1.6]]],
  [26, 3, [[7, 40, 2.2], [9, 20, 3.9], [10, 15, 3.1]]],
  [18, 4, [[11, 60, 1.9], [12, 80, 1.2], [13, 90, 0.8]]],
  [11, 2, [[5, 24, 3.6], [6, 20, 4.8]]],
  [5, 2, [[15, 120, 0.95], [14, 30, 1.1]]],
];
...
export function seed(): Db {
  ...
  const db: Db = {
    ...
    users: [
      ...
      { id: 2, username: 'keeper', password: 'keeper123', name: 'Marco Diaz', role: 'shopkeeper', active: true, createdAt: now - 80 * DAY },
  ...
  PURCHASES.forEach((pp, i) => {
    const items: PurchaseItem[] = pp[2].map((l) => {
      const pr = byId(l[0]);
      return { productId: l[0], sku: pr.sku, name: pr.name, qty: l[1], cost: l[2] };
    });
  ...
  db.purchases.push({
    ...
    byUserId: 2,
    items: [
      { productId: 2, sku: 'TS-1002', name: 'Vintage Graphic Tee', qty: 30, cost: 7.25 },
      { productId: 10, sku: 'CP-4002', name: 'Knit Beanie', qty: 24, cost: 3.1 },
    ],
  ...
  for (let d = 30; d >= 0; d--) {
    ...
    for (let s = 0; s < count; s++) {
      ...
      const method: PayMethod = R() < 0.5 ? 'cash' : R() < 0.7 ? 'card' : 'mobile';
      const paid =
        method === 'cash'
          ? R() < 0.5
            ? Math.ceil(total / 5) * 5
